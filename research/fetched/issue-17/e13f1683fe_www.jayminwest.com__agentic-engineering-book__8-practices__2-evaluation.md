...
## Connections
  ...
  * **To[Debugging](/agentic-engineering-book/8-practices/1-debugging-agents):** Eval failures provide reproducible test cases for debugging. The diagnostic tree in debugging relies on metrics that evaluation surfaces. Fix eval failures via the debugging workflow.
  * **To[Cost and Latency](/agentic-engineering-book/8-practices/3-cost-and-latency):** Evaluation must include cost and latency metrics, not just correctness. Measure cost-per-success (tokens / successful completions) to track efficiency as prompts evolve.
  * **To[Production Concerns](/agentic-engineering-book/8-practices/4-production-concerns):** Production monitoring is continuous evaluation. Metrics tracked in production (error rates, latency, user satisfaction) extend the eval metrics from development.
  * **To[Self-Improving Experts](/agentic-engineering-book/7-patterns/2-self-improving-experts):** The three-role architecture validates design choices through ablation studies—measuring that multi-iteration reflection provides +1.7% improvement over single-pass analysis. Demonstrates why measurement matters for architectural decisions.
  * **To[Model Evaluation](/agentic-engineering-book/3-model/5-model-evaluation):** Model evaluation focuses on choosing and validating models for agentic tasks (benchmarks, metrics theory, LLM-as-judge calibration). This section focuses on evaluating agent implementations using those models (practical workflow, eval set construction, development integration).
  * **To[Human-in-the-Loop](/agentic-engineering-book/7-patterns/6-human-in-the-loop):** Human evaluation calibrates automated judges, catches biases in LLM-as-judge systems, and provides ground truth for subjective quality assessment. Human feedback loops improve both the agent and the evaluation criteria.
...
[PreviousDebugging Agents](/agentic-engineering-book/8-practices/1-debugging-agents)[NextCost and Latency](/agentic-engineering-book/8-practices/3-cost-and-latency)
