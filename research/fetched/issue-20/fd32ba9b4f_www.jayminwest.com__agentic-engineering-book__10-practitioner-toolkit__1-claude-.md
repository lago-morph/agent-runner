...
## Connections
  ...
  * **To[Prompt](/agentic-engineering-book/2-prompt):** Skills and slash commands demonstrate model-invoked vs user-invoked prompt patterns. See [Model-Invoked vs. User-Invoked Prompts](../2-prompt/_index.md#model-invoked-vs-user-invoked-prompts) for design trade-offs.
  * **To[Context](/agentic-engineering-book/4-context):** Skills implement the [Progressive Disclosure pattern](../4-context/3-context-patterns.md#progressive-disclosure-pattern) for context management
  ...
  * **To[Cost and Latency](/agentic-engineering-book/8-practices/3-cost-and-latency):** Token cost models for different feature types—understanding the economics of tools, Skills, subagents, and MCP
...
**See Also** :
  ...
  * [Orchestrator Pattern](/agentic-engineering-book/7-patterns/3-orchestrator-pattern) — Design patterns for flat orchestration
  * [Multi-Agent Context](/agentic-engineering-book/4-context/4-multi-agent-context) — Managing context across agent boundaries
...
**Warning** : Agent teams have known limitations around session resumption, task coordination, and shutdown behavior. See Availability and Status for details.
...
**Too many permission prompts:** Pre-approve common operations in permission settings before spawning teammates to reduce interruptions.

**Teammates stopping on errors:** Check output (Shift+Up/Down or click pane), then either:
...
**Lead shuts down before work done:** Tell lead to keep going. Can also instruct lead to wait for teammates to finish before proceeding.

**Orphaned tmux sessions:** If tmux session persists after team ends:
...
### Connections
  ...
  * **To[Context](/agentic-engineering-book/4-context):** Skills implement the [Progressive Disclosure pattern](../4-context/3-context-patterns.md#progressive-disclosure-pattern)—specialized knowledge loads only when needed, preserving context capacity for other work
  * **To[Tool Use](/agentic-engineering-book/5-tool-use):** Skills teach meta-patterns for tool usage; tools provide capabilities, Skills teach when and how to use them effectively
  * **To[Prompt](/agentic-engineering-book/2-prompt):** Skills are model-invoked rather than user-invoked—the agent determines relevance through reasoning, not explicit commands
...
**Open questions** :
  ...
  * ~~How do hooks determine HEAD vs. subagent context?~~ _[2025-12-09]_ : Hooks receive event data but no explicit HEAD/subagent flag. Detection would require heuristics based on session metadata. Not currently documented or confirmed reliable.
...
**Expandable Details:** Press `Ctrl+O` on the memory indicator to expand and view the full summary content. This shows exactly what context was preserved from previous sessions.
...
**Project-level:** `.claude/rules/*.md` **User-level:** `~/.claude/rules/*.md`
...
**User-Level Rules Default:** All rules in `~/.claude/rules/` automatically apply to every project for that user. No symlinks required—these are global defaults.
...
### Sources
  ...
  * [Patching techniques analysis](/agentic-engineering-book/10-practitioner-toolkit/.claude/.cache/research/external/claude-sneakpeek-patching-techniques-2026-01-30)
...
[ PreviousPractitioner Toolkit](/agentic-engineering-book/10-practitioner-toolkit)[NextGoogle ADK](/agentic-engineering-book/10-practitioner-toolkit/2-google-adk)
