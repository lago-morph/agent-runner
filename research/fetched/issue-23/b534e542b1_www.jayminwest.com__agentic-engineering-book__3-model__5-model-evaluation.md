...
A critical distinction the paper raises: what a system _can_ do versus what it _tends_ to do. Propensities—risk tolerance, communication style, default problem-solving strategies—significantly affect deployment outcomes. This connects directly to model behavioral patterns (see [Model Behavior](/agentic-engineering-book/3-model/2-model-behavior)): two models with identical capability profiles can behave very differently in production due to different propensities.
...
**See Also:**
  ...
  * [Autonomous Loops](/agentic-engineering-book/7-patterns/4-autonomous-loops) — Autoresearch as a specific application of the autonomous loop pattern
  * [Evaluation (Practices)](/agentic-engineering-book/8-practices/2-evaluation) — Eval-driven model improvement workflows
...
## Connections
  ...
  * **To[Evaluation (Practices)](/8-practices/2-evaluation):** Practical evaluation workflows, tooling, and integration with development cycles.
  * **To[Debugging Agents](/8-practices/1-debugging-agents):** Evaluation metrics guide debugging by identifying where in the workflow failures occur. Observability infrastructure supports both evaluation and debugging.
  * **To[Tool Use](/5-tool-use):** Tool calling accuracy is a critical evaluation metric. Tool design quality directly impacts agent performance on benchmarks.
  * **To[Model Selection](/agentic-engineering-book/3-model/1-model-selection):** Evaluation metrics determine which models are suitable for which tasks. Benchmark performance predicts (but doesn't guarantee) production capability.
  * **To[Cost and Latency](/8-practices/3-cost-and-latency):** Evaluation must include cost and latency metrics, not just accuracy. Multi-step workflows amplify both cost and latency beyond single-turn estimates.
...
[PreviousMulti-Model Architectures](/agentic-engineering-book/3-model/4-multi-model-architectures)[NextContext](/agentic-engineering-book/4-context)
