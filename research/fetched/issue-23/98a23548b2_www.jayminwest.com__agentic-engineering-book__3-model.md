...
## Sections
  ...
  * [Model Selection](/agentic-engineering-book/1-model-selection) — How to choose the right model for agentic tasks
  * [Model Behavior](/agentic-engineering-book/2-model-behavior) — How models behave in agentic contexts—variance, consistency, temperature effects, and behavioral patterns
  * [Model Limitations and Workarounds](/agentic-engineering-book/3-model-limitations) — Common model limitations in agentic contexts and practical workarounds—math, hallucination, context limits, instruction drift, and upgrade breakage
  * [Multi-Model Architectures](/agentic-engineering-book/4-multi-model-architectures) — When and how to use multiple models in agent systems—orchestrator patterns, cascades, routing strategies, and planning versus execution separation
  * [Model Evaluation](/agentic-engineering-book/5-model-evaluation) — How to evaluate models for agentic tasks—metrics, benchmarks, observability, and the compound error problem
...
## Connections
  ...
  * **To[Prompt](/2-prompt):** Instruction-following is shaped more by prompt quality than model selection. Different models respond differently to the same prompt, but SOTA models are generally more forgiving.
  * **To[Context](/4-context):** Context length limits shape architecture decisions. Frontier models tend to have larger windows, but the capability-capacity tradeoff still applies.
  * **To[Tool Use](/5-tool-use):** Some models are better at tool-use than others. This is a core selection criterion for agentic work.
  * **To[Practitioner Toolkit](/10-practitioner-toolkit):** Harness availability varies by provider. Model selection and harness selection are coupled decisions for agentic systems. See [Agent Frameworks](/10-practitioner-toolkit/4-agent-frameworks) for harness capability tiers.



[PreviousPrompt Language](/agentic-engineering-book/2-prompt/3-language)[NextModel Selection](/agentic-engineering-book/3-model/1-model-selection)
